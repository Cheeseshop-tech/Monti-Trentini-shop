/* ============================================================
   pricing-core — the shared quote/price/allocation engine
   ------------------------------------------------------------
   Pure functions. No DOM, no client names, no framework. Both the
   wholesale proforma tool and the e-commerce storefront import this.
   Contract: docs/CANONICAL_SCHEMA.md (v1.2).

   Loads as a browser global (window.PricingCore) AND via require().
   ============================================================ */
(function (global) {
  "use strict";
  ...
  /* ---------- PROFORMA / QUOTE SIDE ----------
     $/lb is MERCHANDISE ONLY (FOB × class-of-trade margin).
     Freight & handling are separate line items — never in $/lb. */

  function quoteUnitPrice(sku, opts, config) {
  ...
  function lineLbs(sku, qty) {
    return sku.unit === "lb" ? (Number(qty) || 0) * (sku.pack && sku.pack.netLb || 0) : 0;
  }

  function quoteLineTotal(sku, qty, opts, config) {
  ...
  /* Freight + handling as ORDER-LEVEL LINE ITEMS, volume-gated at 1,500 lb.
     Returns [{id,label,amount}] the proforma lists separately. */
  function freightLines(orderLbs, opts, config) {
    const fr = config.pricing && config.pricing.freight;
    if (!fr || opts.basis !== "delivered") return [];           // pickup => no lines
    orderLbs = Number(orderLbs) || 0;
    if (orderLbs >= fr.thresholdLb) {
      const a = fr.delivered.atOrAboveThreshold;
      return [{ id: "trucking", label: a.label || "Trucking (delivered)",
                amount: round2(a.perLb * orderLbs) }];
    }
  ...
  /* Assemble a full proforma from [{sku, cases}] + one set of order options. */
  function quoteOrder(items, opts, config) {
  ...
  /* ---------- STOREFRONT SIDE ----------
     Published price = active campaign override → else sku.list[audience]. */

  function activeCampaignFor(sku, audience, campaignsDoc, todayISO) {
  ...
  function listPrice(sku, audience, campaignsDoc, todayISO) {
  ...
  function allocate(code, requestedCases, inventory) {
  ...
  const PricingCore = {
    round2, quoteUnitPrice, lineLbs, quoteLineTotal, freightLines, quoteOrder,
    activeCampaignFor, listPrice, allocate,
  };

  if (typeof module !== "undefined" && module.exports) module.exports = PricingCore;
  global.PricingCore = PricingCore;
