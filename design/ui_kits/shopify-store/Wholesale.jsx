...
function WholesalePage({ wholesale, onLogin, onNav }) {
  ...
  if (!wholesale) {
    const benefits = [
      ...
      ["truck", "Chilled delivery", "EXW Elizabeth NJ or delivered — minimum 150 lb per delivery."],
    ...
    <main style={whStyles.acctWrap}>
      <div style={whStyles.acctHead}>
        ...
        <div style={whStyles.acctStats}>
          ...
          <div style={whStyles.acctStat}><div style={whStyles.statN}>150 lb</div><div style={whStyles.statL}>min. delivery</div></div>
        </div>
      </div>
      ...
      <div style={whStyles.reorderGrid}>
        {reorder.map((p) => (
          <div key={p.id} style={whStyles.reorderCard}>
            <div style={{ ...whStyles.reorderThumb, background: p.grad, position: "relative", overflow: "hidden" }}>
              {p.image && <img alt={p.name} {...window.productImgProps(p.image, { w: 120 })} />}
            </div>
            <div style={{ flex: 1 }}>
              <div style={whStyles.reorderName}>{p.name}</div>
              <div style={whStyles.reorderPrice}>{window.money(p.wholesale)} <span style={whStyles.reorderUnit}>/ lb · EXW NJ</span></div>
            </div>
      ...
      <div style={whStyles.table}>
        <div style={{ ...whStyles.trow, ...whStyles.thead }}>
          <span>Item</span><span>Product · format</span><span style={{ textAlign: "right" }}>Net lb/cs</span><span style={{ textAlign: "right" }}>EXW $/lb</span><span style={{ textAlign: "right" }}>Delivered $/lb</span><span></span>
        </div>
        {products.flatMap((p) => (p.formats || []).map((f) => {
          const out = p.availability === "unavailable";
          return (
            <div key={f.code} style={whStyles.trow}>
              <span style={whStyles.tCell}>{f.code}</span>
              <span style={whStyles.tName}>{p.name}<span style={whStyles.tFmt}> · {f.packing}</span></span>
              <span style={{ ...whStyles.tCell, textAlign: "right" }}>{f.netLb}</span>
              <span style={{ ...whStyles.tCell, textAlign: "right", fontWeight: 700, color: "var(--mt-italia-green)" }}>{window.money(f.pickupLb)}</span>
              <span style={{ ...whStyles.tCell, textAlign: "right" }}>{window.money(f.deliveryLb)}</span>
              <span style={{ textAlign: "right" }}>
                {out ? <span style={whStyles.naTag}>N/A</span>
                     : <button style={whStyles.addMini} onClick={() => cart.add(p.id, 6)}>Add ×6</button>}
              </span>
            </div>
          );
        }))}
...
const whStyles = {
  ...
  trow: { display: "grid", gridTemplateColumns: "0.7fr 2.4fr 0.9fr 1fr 1.1fr 1fr", gap: 12, padding: "14px 24px", borderBottom: "1px solid var(--mt-forest-12)", alignItems: "center" },
  ...
  tName: { fontFamily: "var(--font-display)", fontStyle: "italic", fontWeight: 700, fontSize: 17, color: "var(--mt-forest)" },
  tFmt: { fontFamily: "var(--font-ui)", fontStyle: "normal", fontWeight: 500, fontSize: 13, color: "var(--mt-charcoal)" },
  tCell: { fontFamily: "var(--font-ui)", fontWeight: 500, fontSize: 14, color: "var(--mt-forest)" },
  naTag: { fontFamily: "var(--font-ui)", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.05em", color: "#fff", background: "#7b1f24", padding: "4px 9px", borderRadius: 999 },
